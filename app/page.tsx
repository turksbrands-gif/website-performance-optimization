    ...
    <main className="bg-grid relative flex min-h-screen flex-col items-center overflow-x-hidden pt-16 pb-20">
      ...
      <div className="z-10 w-full max-w-[1600px] px-6 grid grid-cols-1 lg:grid-cols-12 gap-8 mt-12">
              ...
              <div className="flex flex-col mt-1">
                <div className="flex items-baseline gap-2">
                  ...
                  <span className={`font-bold text-sm ${
                    sp500Data ? (sp500Data.isUp ? 'text-[#00ffaa]' : 'text-[#ff4d4d]') : 'text-[#00ffaa]'
                  }`}>
        ...
        <section className="col-span-1 lg:col-span-6 flex flex-col items-center text-center mt-4">
          ...
          {/* Form Alanı: Fonksiyonları prop olarak pasladık */}
          <div className="mt-10 w-full">
            <MarketScanForm 
              onScanStart={() => { setIsAnalyzing(true); setAiAnalysis(null); }} 
              onScanComplete={(resultText: string) => { setAiAnalysis(resultText); setIsAnalyzing(false); }} 
            />
          </div>

          <ul className="mt-10 flex flex-wrap items-center justify-center gap-x-8 gap-y-3 text-sm text-muted-foreground">
            {trustPoints.map(({ icon: Icon, label }) => (
              <li key={label} className="flex items-center gap-2"><Icon className="size-4 text-primary" aria-hidden="true" />{label}</li>
            ))}
          </ul>
        </section>
      ...
      {/* --- YENİ EKLENEN YAPAY ZEKA RAPOR TERMINALI --- */}
          ...
          <div className="relative bg-[#11151d]/80 backdrop-blur-xl border border-primary/30 rounded-3xl p-8 shadow-[0_0_50px_rgba(0,255,170,0.07)] overflow-hidden">
            ...
            {isAnalyzing ? (
              ...
              <div className="prose prose-invert max-w-none text-left text-gray-300 text-sm leading-relaxed space-y-4
                prose-headings:text-white prose-headings:font-bold prose-h1:text-2xl prose-h2:text-xl prose-h3:text-lg prose-headings:mt-6 prose-headings:mb-3
                prose-ul:list-disc prose-ul:pl-6 prose-li:mb-1.5 prose-strong:text-primary prose-a:text-blue-400 hover:prose-a:underline">
